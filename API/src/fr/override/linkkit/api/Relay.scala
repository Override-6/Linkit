package fr.`override`.linkkit.api

import fr.`override`.linkkit.api.`extension`.{RelayExtensionLoader, RelayProperties}
import fr.`override`.linkkit.api.exception.RelayInitialisationException
import fr.`override`.linkkit.api.packet.{PacketManager, TrafficHandler}
import fr.`override`.linkkit.api.packet.channel.PacketChannel
import fr.`override`.linkkit.api.packet.collector.PacketCollector
import fr.`override`.linkkit.api.system.config.RelayConfiguration
import fr.`override`.linkkit.api.system.event.EventObserver
import fr.`override`.linkkit.api.system.security.RelaySecurityManager
import fr.`override`.linkkit.api.system.{JustifiedCloseable, RemoteConsole, Version}
import fr.`override`.linkkit.api.task.TaskScheduler
...
object Relay {
    final val apiVersion = Version("Api", "0.8.0", stable = false)
}
...
trait Relay extends JustifiedCloseable with TaskScheduler {
     ...
     * A PacketManager can register a [[fr.`override`.linkkit.api.packet.Packet]]
     * then build or decompose a packet using [[fr.`override`.linkkit.api.`extension`.packet.PacketFactory]]
     *
     ...
     *
     * @see [[fr.`override`.linkkit.api.exception.IllegalPacketWorkerLockException]]
     * */
