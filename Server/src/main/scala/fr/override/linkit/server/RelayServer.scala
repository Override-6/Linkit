...
import fr.`override`.linkit.server.config.{AmbiguityStrategy, ExternalConnectionConfiguration}
import fr.`override`.linkit.server.connection.{ConnectionInitialisationException, ExternalConnectionsManager, ServerExternalConnection, SocketContainer}
...
class RelayServer private[server](override val configuration: ExternalConnectionConfiguration) extends Relay {
    ...
    private[server] val connectionsManager = new ExternalConnectionsManager(this)
    ...
    override val securityManager: ServerSecurityManager = configuration.hasher
    ...
    * */
    ...
    private def handleConnectionIdAmbiguity(current: ServerExternalConnection,
        ...
        if (!current.isConnected) {
            ...
            println(s"The connection of ${current.supportIdentifier} has been resumed.")
            return
        }

        val identifier = current.supportIdentifier
