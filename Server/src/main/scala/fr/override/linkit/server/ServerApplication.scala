...
class ServerApplication private(override val configuration: ServerApplicationConfiguration) extends ApplicationContext {
    ...
    override def shutdown(): Unit = this.synchronized {
        ...
        * */
        mainWorkerPool.checkCurrentThreadOwned("Shutdown must be performed into Application's pool")
        ensureAlive()
    ...
    private def start(): Unit = {
        mainWorkerPool.checkCurrentThreadOwned("Start must be performed into Application's pool")
        if (alive) {
