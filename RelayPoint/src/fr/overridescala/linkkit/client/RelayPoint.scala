...
object RelayPoint {
      val version: Version = Version("RelayPoint", "0.5.1", stable = false)
...
class RelayPoint(override val configuration: RelayPointConfiguration) extends Relay {
      ...
      def checkIdentifierSyntax(): Unit = {
            if (identifier == ServerID || identifier == "unknown")
                  throw RelayInitialisationException(s"'$identifier' is a blacklisted identifier !")
            if (!identifier.matches("^\\w{0,16}$"))
                  throw RelayInitialisationException(s"'$identifier' does not match regex '^\\w{0,16}$$'")
      }

      override def start(): Unit = {
            checkIdentifierSyntax()
