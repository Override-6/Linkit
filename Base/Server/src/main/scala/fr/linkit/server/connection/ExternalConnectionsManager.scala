...
class ExternalConnectionsManager(server: ServerConnection) extends JustifiedCloseable {
    ...
    private val connections   = mutable.HashMap.empty[NameTag, ServerExternalConnection]
    ...
    override def close(reason: Reason): Unit = {
        for ((_, connection) <- connections) try {
            AppLoggers.App.info(s"Shutting down connection '${connection.boundNT}'...")
            connection.shutdown()
        } catch {
    ...
    @throws[ConnectionException]("if the provided identifier is already taken")
    @throws[ServerException]("if the registered connection count exceeded configuration limit.")
    def registerConnection(identifier: NameTag,
    ...
    def broadcastPacket(result: PacketUpload, discardedIDs: String*): Unit = {
        ...
        val candidates = connections.values
                .filter(con => !discardedIDs.contains(con.boundNT) && con.isConnected)
     ...
     * */
    def unregister(identifier: NameTag): Option[ServerExternalConnection] = {
     ...
     *
     * @param name the identifier linked [[ServerExternalConnection]]
     * @return the found [[ServerExternalConnection]] bound with the identifier
     * */
    @Nullable
    def getConnection(name: NameTag): ServerExternalConnection = connections.get(name).orNull

    def countConnections: Int = connections.size

    def listNames: Seq[NameTag] = connections.keys.toSeq
    ...
    private[connection] def deflect(result: PacketDownload): Unit = {
        ...
        val target = coords match {
            case e: DedicatedPacketCoordinates => e.targetNT
            case _                             =>
