...
import linkit.base.resource.local.LocalResourceFolder.ForbiddenChars
...
abstract class BaseResourceFolder(parent: Option[ResourceFolder], listener: ResourceListener, adapter: Path) extends AbstractResource(parent, adapter) with ResourceFolder {
    
    protected val resources  = new mutable.HashMap[String, Resource]()
    protected val maintainer = this.synchronized {
    ...
    override def register[R <: Resource](name: String, factory: ResourceFactory[R]): R = this.synchronized {
        ...
        val resource     = factory(adapter, listener, Some(this))
