...
import fr.linkit.engine.gnom.cache.sync.generation.sync.ScalaBlueprintUtilities.toScalaString
...
class StaticsCallerClassBlueprint(bp: InputStream) extends AbstractClassBlueprint[SyncStaticsDescription[_]](bp) {
    ...
    override val rootScope   : RootValueScope = new RootValueScope {
        ...
        bindSubScope(new SyncMethodBlueprint.ValueScope("INHERITED_METHODS", _, _) {
            bindValue("MethodName" ~> getMethodName)
            bindValue("ParamsOutMatch" ~> getParamsOut)
        }, (c, f: MethodDescription => Unit) => {
            c.listMethods().foreach(f)
        })
    }

    private def getMethodName(desc: MethodDescription): String = {
