...
import fr.linkit.engine.gnom.cache.sync.tree.{InitializedConnectedObjectTree, NoSuchConnectedObjectTreeException}
    ...
    override lazy val value: AnyRef = {
        ...
        selector.findObject(cacheRef).getOrElse {
            ...
            case cache: ConnectedObjectCache[_] =>
                ...
                val connected = tree.connectObject(nodePath.dropRight(1), stubClassDef, ref.owner, SyncLevel.Mirror, nodePath.last.id).obj
                connected
        }
    }
}