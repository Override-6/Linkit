...
import fr.linkit.engine.gnom.cache.sync.contract.behavior.LeveledSBDN._
...
class LeveledSBDN[A <: AnyRef](@Nullable val descriptor: StructureContractDescriptor[A],
    ...
    def getContract(clazz: Class[_ <: A], context: ConnectedObjectContext): StructureContract[A] = {
        ...
        val mirroringInfo = descriptor match {
            ...
            case _                                                   =>
                if (context.syncLevel == Mirroring) Some(autoDefineMirroringInfo(clazz)) else None
        }
...
object LeveledSBDN {
    ...
    private[behavior] def autoDefineMirroringInfo(clazz: Class[_]): MirroringInfo = {
