...
class LambdaModifier[A] extends MethodCompModifier[A] {

    protected var currentToCurrent: (A, LocalMethodInvocation[_]) => A         = (localParam, _) => localParam
    protected var remoteToCurrent : (A, LocalMethodInvocation[_], Engine) => A = (receivedParam, _, _) => receivedParam
    protected var toRemote        : (A, LocalMethodInvocation[_], Engine) => A = (localParam, _, _) => localParam
