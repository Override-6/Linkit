...
class LanguageDescriptorDataHolder(file: String, propertyClass: PropertyClass) extends ContractDescriptorDataHolder {

    override def getData(network: Network): ContractDescriptorData = Contract(file)(network.connection.getApp, propertyClass)
}