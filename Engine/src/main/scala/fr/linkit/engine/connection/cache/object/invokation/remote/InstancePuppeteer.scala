...
package fr.linkit.engine.connection.cache.`object`.invokation.remote
...
import fr.linkit.engine.connection.cache.`object`.invokation.local.ObjectChip
import fr.linkit.engine.connection.cache.`object`.tree.PuppetNode
import fr.linkit.engine.connection.cache.`object`.{NoSuchPuppetNodeException, ThrowableWrapper}
...
class InstancePuppeteer[S](channel: RequestPacketChannel,
    ...
    override def synchronizedObj(obj: Any, id: Int): Any = {
        ...
        repo.genSynchronizedObject(objPath, obj, currentIdentifier, defaults)
    }
