...
class SimpleAsyncTask[A](override val taskID: Int, override val parent: Option[WorkerTask[_] with AsyncTaskController], task: () => Try[A]) extends WorkerTask[A] with AsyncTaskController {
...
object SimpleAsyncTask {
    ...
    def apply[A](taskID: Int, parent: WorkerTask[_] with AsyncTaskController)(task: => Try[A]): SimpleAsyncTask[A] = new SimpleAsyncTask(taskID, Option(parent), () => task)
