package fr.`override`.linkit.core.internal.concurrency

import RelayThreadPool.{RelayThread, checkCurrentIsWorker}
 ...
 * (see [[fr.`override`.linkit.skull.connection.packet.traffic.PacketInjectable]] for further details about injection
 ...
 * (see [[fr.`override`.linkit.skull.connection.packet.traffic.PacketInjections.PacketInjection]])
 * {{{
 ...
 * */
class RelayThreadPool(val prefix: String, val nThreads: Int) extends AutoCloseable {
...
object RelayThreadPool {
    ...
    /**
     * @return Some if the current thread is a member of a [[RelayThreadPool]], None instead
     * */
    def currentPool(): Option[RelayThreadPool] = {
        currentThread match {
