...
import fr.`override`.linkit.skull.internal.plugin.LoadPhase._
import fr.`override`.linkit.skull.internal.plugin.RelayExtensionLoader.{MainClassField, PropertyName}
import fr.`override`.linkit.skull.internal.system.RelayException
import fr.`override`.linkit.skull.internal.system.fsa.FileAdapter
import fr.`override`.linkit.core.internal.plugin.fragment.{ExtensionFragment, RemoteFragment}
import fr.`override`.linkit.internal.plugin.fragment.{ExtensionFragment, RemoteFragment}
 ...
 * </p>
 * <p>
 * An extension can add multiple [[ExtensionFragment]]
 * in order to provide some of its aspects to other relay's extension, or even to other connected relays
 * if the added extension fragment is an instance of [[RemoteFragment]]
 * </p>
 *
 * @see [[LinkitPlugin]]
 * */
...
class RelayExtensionLoader(relay: Relay) extends Closeable {
    ...
    val fragmentHandler = new fragment.FragmentHandler(relay, this)

    private val loadedExtensions = ListBuffer.empty[LinkitPlugin]
    ...
    private def loadJar(classLoader: ClassLoader, path: FileAdapter): Class[_ <: LinkitPlugin] = {
    ...
    def loadExtension[T <: LinkitPlugin](clazz: Class[T]): T = {
        if (phase == CLOSE)
    ...
    def loadExtensions(classes: Class[_ <: LinkitPlugin]*): Unit =
        loadAllExtensions(classes)

    private def loadAllExtensions(extensions: Seq[Class[_ <: LinkitPlugin]]): Unit = {
        ...
        def perform(action: LinkitPlugin => Unit): Unit = {
