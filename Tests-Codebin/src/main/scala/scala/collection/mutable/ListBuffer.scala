...
class ListBuffer[A]
    ...
    def iterator: Iterator[A] = new mutable.MutationTracker.CheckedIterator(first.iterator, mutationCount)
...
object ListBuffer extends StrictOptimizedSeqFactory[ListBuffer] {
    ...
    def newBuilder[A]: mutable.Builder[A, ListBuffer[A]] = new GrowableBuilder(empty[A])
